...
function getPositiveArrayValues(arr) {
  ...
  const arrayValues = Array.from(valuesIterator);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map(value => value ** 2);
  
  const filteredSquaredArray = squaredArray.filter(value => value > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);

  lastNElements.forEach(value => console.log(value));
